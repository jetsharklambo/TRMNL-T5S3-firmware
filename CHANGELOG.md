...
### 🔧 Critical Bug Fixes - Display Rendering & Stability

This release fixes critical rendering artifacts and heap corruption crashes in the FastEPD library integration.

### Fixed

#### Display System
- **Fixed rendering artifacts** - Eliminated repeated vertical strip on right edge and missing pixels on left edge
  - Root cause: FastEPD v1.3.0 has buffer calculation bugs in 4BPP mode
  - Applied 15 local patches to `.pio/libdeps/T5_E_PAPER_S3_V7/FastEPD/src/FastEPD.inl`
  - Changed 13 buffer calculations from `/4` (2BPP) to `/2` (4BPP) divisor
  - Fixed buffer allocation, indexing, memset/memcpy operations, and write operations

- **Fixed heap corruption crash** - Device now enters deep sleep successfully without crashes
  - Root cause: DMA buffer allocated only 480 bytes but required 960 bytes for double-buffering
  - Doubled DMA buffer allocation from `(width/2)` to `(width)` at line 1295 in FastEPD.inl
  - Eliminates "CORRUPT HEAP: Bad head at 0x3fcb5fa8" crash during WiFi shutdown

### Added

#### Display Configuration
- **IMAGE_X_OFFSET configuration** - 5px horizontal offset for better bezel centering
  - Configurable in `include/config.h`
  - Applied during PNG rendering callback in `display.cpp:199`
  - Improves visual balance with device bezels

#### Documentation
- **FastEPD Library Patches section** in CLAUDE.md - Complete technical documentation of all 15 patches
- **Updated KNOWN_ISSUES.md** - Moved fixed issues to historical reference, marked all critical issues as resolved
- **Enhanced Development Notes** - Added guidance for IMAGE_X_OFFSET configuration and RENDER_MODE testing

### Changed

#### Dependencies
- **FastEPD v1.3.0** - Now uses locally patched version
  - ⚠️ **DO NOT upgrade** without reapplying patches or confirming upstream fixes
  - See CLAUDE.md for complete patch details and rationale

#### Display Rendering
- **Rendering modes** - 5 configurable modes documented (RENDER_MODE 1-5)
...
**Patch Locations** (FastEPD.inl):
- Buffer allocation: lines 1288-1289, 1295
- Row operations: lines 1553, 1562, 1575, 1577, 1579, 1635, 1636, 1672
- Temp buffer indexing: lines 1610, 1633, 1758, 1796
- Loop iterations: lines 1614, 1621, 1655, 1663, 1762, 1769, 1820, 1828
- Write operations: lines 1799, 1800, 1846, 1848, 1849
...
- ✅ Minimal memory cost (~480 bytes for DMA buffer, negligible on ESP32-S3)

### Build Information
- **Version**: 1.1.0
- **Platform**: ESP32-S3 (LilyGo T5 4.7")
- **FastEPD**: 1.3.0 (locally patched)
- **Panel Config**: BB_PANEL_EPDIY_V7

### Testing
- ✅ Rendering artifacts eliminated on 960x540 display
- ✅ Heap corruption crash resolved
- ✅ Deep sleep entry/exit stable
- ✅ WiFi shutdown sequence completes successfully
- ✅ Battery monitoring functional
- ✅ Multiple sleep/wake cycles tested

### Migration from v1.0.0
No user action required - this is a bug fix release. Flash v1.1.0 firmware using standard upload procedure.

### For Upstream TRMNL Repository
This release is ready for upstream integration. The FastEPD patches are well-documented and necessary for stable operation on the LilyGo T5S3 Pro hardware. A comparison variant using unpatched public FastEPD library is available to demonstrate the bugs.
...
- Upstream FastEPD library fix (if/when maintainer accepts patches)
